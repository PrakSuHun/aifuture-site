...
**관리자 화면이 사이트와 똑같이 보이고, 텍스트·이미지를 클릭해서 직접 편집**하는 방식입니다.

### 첫 사용
1. `사이트URL/admin/` 접속 (예: `https://aifuture-site-b7n3.vercel.app/admin/`)
2. **GitHub Personal Access Token** 입력 (없으면 화면의 발급 링크 클릭)
3. 로그인하면 **사이트 화면 + 상단 검정 툴바**가 나타남 — 이게 편집 모드

### 편집 절차
- **텍스트 편집**: 글자 위에 마우스 올리면 시안색 테두리 → 클릭 → 우측 사이드바에 입력칸 → 수정 후 **적용 (미리보기)**
- **이미지 편집**: 이미지 위에 호버하면 노란색 테두리 → 클릭 → 사이드바에서 **파일 업로드** 또는 URL 입력
- 변경사항은 노란색 테두리로 표시됨 (아직 미저장)
- 화면 변경을 모두 완료한 뒤 우측 상단 **저장 (커밋)** 클릭 → GitHub에 자동 커밋
- 1~2분 후 Vercel 자동 재배포 → 모든 방문자에게 반영

### 종료
- 상단 **미리보기** 버튼: ?edit=1 빠지고 일반 사이트로
- 상단 **로그아웃** 버튼: 토큰 삭제 후 로그인 화면으로
